...
create table portuguese_words (
  ...
  category text not null check (category in ('cumprimentos', 'numeros', 'comida', 'viagem', 'frases', 'semana', 'verbos', 'conversa', 'negocios')),
  pt text not null,
  en text,
  ja text,
  note text,
...
insert into portuguese_words (category, pt, en, ja, note, sort_order) values
  ...
  -- semana 曜日・週
  ('semana', 'a semana',       'week',      '週',     null, 1),
  ('semana', 'segunda-feira',  'Monday',    '月曜日',
   'segunda = second（第二の） + feira = market day（市の日） → "2nd day"（日曜日から数えて）', 2),
  ('semana', 'terça-feira',    'Tuesday',   '火曜日',
   'terça = third（第三の） + feira → "3rd day"', 3),
  ('semana', 'quarta-feira',   'Wednesday', '水曜日',
   'quarta = fourth（第四の） + feira → "4th day"（quatro = 4 と同じ語源）', 4),
  ('semana', 'quinta-feira',   'Thursday',  '木曜日',
   'quinta = fifth（第五の） + feira → "5th day"（cinco = 5 と同じ語源）', 5),
  ('semana', 'sexta-feira',    'Friday',    '金曜日',
   'sexta = sixth（第六の） + feira → "6th day"（seis = 6 と同じ語源）', 6),
  ('semana', 'sábado',         'Saturday',  '土曜日',
   '安息日（Sabbath）が語源 — 数字ではない', 7),
  ('semana', 'domingo',        'Sunday',    '日曜日',
   'ラテン語 dies Dominicus（主の日）が語源 — 数字ではない', 8),
  ('semana', 'o fim de semana', 'weekend',  '週末',
   'fim = end（終わり） / de = of / semana = week（週）', 9),
  ('semana', 'hoje',           'today',     '今日', null, 10),
  ('semana', 'amanhã',         'tomorrow',  '明日', null, 11),
  ('semana', 'ontem',          'yesterday', '昨日', null, 12),
  -- verbos 動詞
  ...
  ('negocios', 'prazo de entrega', 'delivery deadline',       '納期',
   'prazo = deadline / time limit（期限） / de = of（〜の） / entrega = delivery（納品・配達, entregar = to deliver） → "deadline of delivery"', 11),
