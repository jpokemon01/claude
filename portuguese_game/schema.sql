...
insert into portuguese_words (category, pt, en, ja, note, sort_order) values
  ...
  -- comida 食べ物
  ('comida', 'o café da manhã', 'breakfast', '朝食',
   'café = coffee（コーヒー） / da manhã = of the morning（朝の, de + a） → "morning coffee"', 1),
  ('comida', 'o almoço',    'lunch',  '昼食', null, 2),
  ('comida', 'o jantar',    'dinner', '夕食', null, 3),
  ('comida', 'o churrasco', 'Brazilian barbecue', 'シュラスコ（ブラジル式焼肉）', null, 4),
  ('comida', 'a feijoada',  'feijoada (black bean & meat stew)', 'フェイジョアーダ（豆と肉の煮込み）',
   'feijão = beans（豆）が語源 — ブラジルの国民食', 5),
  ('comida', 'o suco de laranja', 'orange juice', 'オレンジジュース',
   'suco = juice（ジュース） / de = of / laranja = orange（オレンジ）', 6),
  ('comida', 'a sobremesa', 'dessert', 'デザート',
   'sobre = over（〜の上に） / mesa = table（テーブル） → "after the meal, on the table"', 7),
  ('comida', 'o legume',    'vegetable', '野菜',  null, 8),
  ('comida', 'o queijo',    'cheese',    'チーズ', null, 9),
  ('comida', 'o açúcar',    'sugar',     '砂糖',  null, 10),
  ('comida', 'o sal',       'salt',      '塩',    null, 11),
  ('comida', 'Está delicioso!', 'It''s delicious!', 'おいしいです！',
   'está = is（estar） / delicioso = delicious（おいしい）', 12),
  ('comida', 'Estou com fome', 'I''m hungry', 'お腹がすきました',
   'estou = I am（estar） / com = with（〜と共に） / fome = hunger（空腹） → "I am with hunger"', 13),
  -- viagem 旅行
  ('viagem', 'a praia',      'beach',    'ビーチ', null, 1),
  ('viagem', 'a passagem',   'ticket (plane / bus)', '航空券・乗車券',
   'passar = to pass（通る）が語源 → "a passing"', 2),
  ('viagem', 'o voo',        'flight',   'フライト（便）',
   'voar = to fly（飛ぶ）が語源', 3),
  ('viagem', 'a mala',       'suitcase', 'スーツケース', null, 4),
  ('viagem', 'o passaporte', 'passport', 'パスポート',   null, 5),
  ('viagem', 'a reserva',    'reservation', '予約',
   'reservar = to reserve（予約する）が語源', 6),
  ('viagem', 'o câmbio',     'currency exchange', '両替・為替', null, 7),
  ('viagem', 'a diária',     'daily rate (hotel)', '宿泊料金（1泊分）',
   'dia = day（日）が語源 → "per-day charge"', 8),
  ('viagem', 'Que horas sai o ônibus?', 'What time does the bus leave?', 'バスは何時に出ますか？',
   'que horas = what time（何時） / sai = leaves（sair の活用） / o ônibus = the bus', 9),
  ('viagem', 'Pode me levar ao hotel?', 'Can you take me to the hotel?', 'ホテルまで連れて行ってもらえますか？',
   'pode = can（poder） / me levar = take me（私を連れて行く） / ao = to the（a + o） / hotel', 10),
  ('viagem', 'É longe daqui?', 'Is it far from here?', 'ここから遠いですか？',
   'é = is（ser） / longe = far（遠い） / daqui = from here（de + aqui, ここから）', 11),
  ('viagem', 'Estou perdido', 'I am lost', '道に迷いました',
   'estou = I am（estar） / perdido = lost（perder = to lose の過去分詞）', 12),
  -- frases 便利なフレーズ
